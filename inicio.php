...
body{
    ...
    min-height:100vh;
    font-size:18px; /* aumenta todo o texto */
}

/* wrapper maior e mais espaçoso */
.dashboard_container{
    width: calc(100% - 48px);
    max-width: 1100px; /* aumenta a área */
    margin: 0 auto;
    background:var(--card);
    padding:50px 50px; /* mais área interna */
...
form table{
    ...
    margin:14px 0;
    font-size:16px; /* aumentar texto da tabela */
}
form th, form td{
    padding:14px 12px; /* mais espaço nas células */
    text-align:left;
...
input[type="time"], input[type="text"], input[type="email"], input[type="password"]{
    width:100%;
    padding:10px 12px; /* padding maior */
    border:1px solid #d6e1ea;
    border-radius:8px;
    background:#fff;
    font-size:16px; /* aumenta o texto dos inputs */
    color:#123;
    transition:box-shadow .15s, border-color .15s;
}
input[type="time"]{
    font-size:16px; /* números maiores no time */
}
...
input[type="submit"]{
    ...
    color:#fff;
    border:0;
    padding:12px 22px; /* aumenta o botão */
    border-radius:10px;
    cursor:pointer;
    font-weight:700;
    font-size:16px;
...
@media (max-width:900px){
    .dashboard_container{ padding:20px; width:calc(100% - 32px); }
    form table{ display:block; overflow:auto; white-space:nowrap; }
    form th, form td{ min-width:180px; }
}
